...
const dataDir = path.join(outputDir, '../src/assets/data-tw');

await fs.mkdir(dataDir, { recursive: true });

console.log(chalk.bgWhite('===================================='));

const wordlist = LangZh.wordlist('tw');
...
async function readAndParseFile(filePath) {
  ...
  console.log('====================================');
  return original.filter(item => wordlist.getWordIndex(item.character) != -1);
}
...
console.log('length', dictionaryItems.length, graphicsItems.length);
console.assert(dictionaryItems.length === 2048, 'length mismatch 2048');
console.assert(
